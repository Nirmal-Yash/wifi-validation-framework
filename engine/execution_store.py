...
def connect() -> sqlite3.Connection:
    ...
    conn.execute("PRAGMA foreign_keys=ON"); conn.execute("PRAGMA busy_timeout=10000"); migrate(conn)
    return conn


def audit(event_type: str, actor: str | None, message: str, payload: dict | None = None) -> None:
    with connect() as conn:
        conn.execute("INSERT INTO audit_events(event_type,actor,message,payload_json) VALUES(?,?,?,?)", (event_type, actor or "system", message, json.dumps(payload or {}, sort_keys=True, default=str))); conn.commit()


def topology_snapshot():
    path = ROOT / "config" / "devices.yaml"; content = path.read_text(encoding="utf-8") if path.exists() else None
    digest = hashlib.sha256(content.encode("utf-8")).hexdigest() if content is not None else None
    with connect() as conn:
        row = conn.execute("SELECT tv.id,tv.version FROM topology_versions tv JOIN topologies t ON t.id=tv.topology_id WHERE tv.status='ACTIVE' ORDER BY tv.id DESC LIMIT 1").fetchone()
...
def create_execution(firmware_version="1.0.0", suite_name="live", triggered_by="api", environment="tier1", notes="", command="", pytest_args=None):
    topology_hash, topology_version_id, snapshot = topology_snapshot(); args_json = json.dumps(pytest_args, separators=(",", ":")) if pytest_args is not None else None
    with connect() as conn:
        ensure_firmware(conn, firmware_version)
        cur = conn.execute("""INSERT INTO executions(firmware_version,topology_version_id,mode,status,suite_name,triggered_by,environment,notes,topology_hash,topology_snapshot,command,phase,pytest_args_json)
            VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)""", (firmware_version, topology_version_id, environment, "QUEUED", suite_name, triggered_by, environment, notes, topology_hash, snapshot, command, "QUEUED", args_json))
        eid = int(cur.lastrowid); add_event(conn, eid, "QUEUED", "Execution queued", {"actor": triggered_by}); conn.commit()
    audit("EXECUTION_CREATED", triggered_by, f"Execution #{eid} queued", {"execution_id": eid, "firmware": firmware_version, "environment": environment, "suite": suite_name}); return eid


def add_event(conn, eid, event_type, message, payload=None): conn.execute("INSERT INTO execution_events(execution_id,event_type,message,payload_json) VALUES(?,?,?,?)", (eid, event_type, message, json.dumps(payload or {}, sort_keys=True, default=str)))
...
def set_status(eid, status, message=None, **fields):
    ...
    if db_status in {"PASSED","FAILED","ERROR","CANCELLED"}: assignments.append("finished_at=COALESCE(finished_at,CURRENT_TIMESTAMP)")
...
def record_artifact(eid,source,kind,test_result_id=None):
    ...
    with connect() as conn:cur=conn.execute("INSERT INTO evidence(execution_id,test_result_id,kind,path,sha256) VALUES(?,?,?,?,?)",(eid,test_result_id,kind,rel,digest));conn.commit()
    return {"id":cur.lastrowid,"execution_id":eid,"kind":kind,"path":rel,"sha256":digest}


def record_metric(eid,metric_name,metric_value,metric_unit="",test_result_id=None):
    with connect() as conn:cur=conn.execute("INSERT INTO execution_metrics(execution_id,test_result_id,metric_name,metric_value,metric_unit) VALUES(?,?,?,?,?)",(eid,test_result_id,metric_name,float(metric_value),metric_unit));conn.commit()
...
def record_test_results(eid,junit_path):
    ...
    with connect() as conn:
        ...
        for case in cases:
            ...
            counts[bucket]+=1; duration=float(case.get("time") or 0)
            conn.execute("INSERT INTO test_results(execution_id,test_name,test_file,status,duration,duration_ms,error_message,failure_phase) VALUES(?,?,?,?,?,?,?,?)",(eid,case.get("name") or "unknown",case.get("classname") or "",status,duration,duration*1000,message,""))
