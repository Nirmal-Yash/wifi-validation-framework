...
def topology_snapshot() -> tuple[str | None, int | None, str | None]:
    ...
    with connect() as conn:
        row = conn.execute(
            "SELECT tv.id, tv.version FROM topology_versions tv JOIN topologies t ON t.id=tv.topology_id WHERE tv.status='ACTIVE' ORDER BY tv.id DESC LIMIT 1"
        ).fetchone()
...
def create_execution(firmware_version: str = "1.0.0", suite_name: str = "live", triggered_by: str = "api", environment: str = "tier1", notes: str = "", command: str = "") -> int:
    topology_hash, topology_version_id, snapshot = topology_snapshot()
    with connect() as conn:
        ensure_firmware(conn, firmware_version)
        cur = conn.execute(
            """INSERT INTO executions
            (firmware_version, topology_version_id, mode, status, suite_name, triggered_by,
             environment, notes, topology_hash, topology_snapshot, command, phase)
            VALUES (?, ?, ?, 'QUEUED', ?, ?, ?, ?, ?, ?, ?, 'QUEUED')""",
            (firmware_version, topology_version_id, environment, suite_name, triggered_by, environment, notes, topology_hash, snapshot, command),
        )
...
def add_event(conn: sqlite3.Connection, execution_id: int, event_type: str, message: str, payload: dict[str, Any] | None = None) -> None:
    conn.execute("INSERT INTO execution_events(execution_id,event_type,message,payload_json) VALUES(?,?,?,?)", (execution_id, event_type, message, json.dumps(payload or {}, sort_keys=True)))
...
def set_status(execution_id: int, status: str, message: str | None = None, **fields: Any) -> None:
    ...
    # Legacy schema only permits QUEUED/RUNNING/PASSED/FAILED/ERROR/CANCELLED.
    db_status = "RUNNING" if status in {"PROVISIONING", "COLLECTING", "ANALYZING"} else status
    assignments = ["status=?", "phase=?"]
    values: list[Any] = [db_status, status]
    if db_status == "RUNNING":
        assignments.append("started_at=COALESCE(started_at, CURRENT_TIMESTAMP)")
    if db_status in {"PASSED", "FAILED", "ERROR", "CANCELLED"}:
    ...
    with connect() as conn:
        ...
        add_event(conn, execution_id, status, message or f"Execution phase changed to {status}", fields)
        conn.commit()


def mark_cancel_requested(execution_id: int) -> bool:
    with connect() as conn:
        cur = conn.execute("UPDATE executions SET cancel_requested=1 WHERE id=? AND status IN ('QUEUED','RUNNING')", (execution_id,))
        conn.commit()
...
def list_executions(limit: int = 50, status: str | None = None) -> list[dict[str, Any]]:
    ...
    with connect() as conn:
        rows = conn.execute("SELECT * FROM executions WHERE status=? ORDER BY id DESC LIMIT ?", (status, limit)).fetchall() if status else conn.execute("SELECT * FROM executions ORDER BY id DESC LIMIT ?", (limit,)).fetchall()
    ...
    with connect() as conn:
        ...
        for case in cases:
            ...
            if failure is not None:
                status, bucket, message = "FAILED", "failed", failure.get("message") or (failure.text or "")
            elif error is not None:
                status, bucket, message = "ERROR", "errors", error.get("message") or (error.text or "")
            elif skipped is not None:
                status, bucket, message = "SKIPPED", "skipped", skipped.get("message") or ""
            else:
                status, bucket, message = "PASSED", "passed", ""
            counts[bucket] += 1
            classname, name = case.get("classname") or "", case.get("name") or "unknown"
            duration = float(case.get("time") or 0.0)
            conn.execute("INSERT INTO test_results(execution_id,test_name,test_file,status,duration,duration_ms,error_message,failure_phase) VALUES(?,?,?,?,?,?,?,?)", (execution_id, name, classname, status, duration, duration * 1000.0, message, ""))
        conn.execute("UPDATE executions SET total_tests=?,passed=?,failed=?,blocked=?,skipped=?,errors=? WHERE id=?", (counts["total"], counts["passed"], counts["failed"], counts["blocked"], counts["skipped"], counts["errors"], execution_id))
        conn.commit()
    return counts
