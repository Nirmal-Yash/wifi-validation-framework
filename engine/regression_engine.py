...
def compute_baseline(metric_name: str, topology_version_id: int | None = None, window: int = DEFAULT_WINDOW) -> dict[str, Any] | None:
...
def analyze_execution(execution_id: int, threshold_percent: float = DEFAULT_THRESHOLD_PERCENT) -> list[dict[str, Any]]:
    """Compare all metrics from an execution against recent history.

    Alerts are suppressed until MIN_SAMPLES historical samples exist. This avoids
    noisy regressions during initial platform bootstrap.
    """
    with connect() as conn:
    ...
    for row in current:
        baseline = compute_baseline(row["metric_name"], row["topology_version_id"])
        ...
        with connect() as conn:
            baseline_row = conn.execute(
                "SELECT id FROM baselines WHERE name=? AND firmware_version=?",
                (f"auto:{row['metric_name']}", "rolling"),
            ).fetchone()
            if baseline_row is None:
                conn.execute(
                    "INSERT OR IGNORE INTO firmware_metadata(firmware_version) VALUES ('rolling')"
                )
                conn.execute(
                    "INSERT OR IGNORE INTO baselines(name,firmware_version,topology_version_id) VALUES(?,?,?)",
                    (f"auto:{row['metric_name']}", "rolling", None),
                )
            ...
            cur = conn.execute(
                "INSERT INTO regressions(baseline_id,execution_id,test_name,baseline_status,candidate_status,severity) VALUES(?,?,?,?,?,?)",
                (baseline_row["id"], execution_id, "metric", "BASELINE", "REGRESSION", severity),
            )
            regression_id = cur.lastrowid
            conn.execute(
                "INSERT INTO regression_metrics(regression_id,metric_name,baseline_value,current_value,delta_percent,threshold_percent) VALUES(?,?,?,?,?,?)",
                (regression_id, row["metric_name"], baseline["baseline_value"], comparison["current_value"], comparison["delta_percent"], threshold_percent),
            )
            conn.commit()
