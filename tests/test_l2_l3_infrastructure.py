...
def test_8021q_vlan_segregation(system_config, async_sniffer):
    ...
    try:
        ...
        subprocess.run(f"sudo ip netns exec {ap['namespace']} ip addr replace 10.20.30.1/24 dev {vlan_iface}", shell=True, check=True)
        route_check = subprocess.run(
            f"sudo ip netns exec {ap['namespace']} ip route show dev {vlan_iface}",
            shell=True,
            capture_output=True,
            text=True,
            check=True,
...
def test_etherchannel_lacp_negotiation(system_config, async_sniffer):
    """Validate that the Linux bonding driver exposes IEEE 802.3ad mode."""
    ap = system_config['nodes']['ap']
    try:
        subprocess.run(
            f"sudo ip netns exec {ap['namespace']} ip link add bond0 type bond mode 802.3ad",
            shell=True,
            check=True,
        )
        mode = subprocess.run(
            f"sudo ip netns exec {ap['namespace']} cat /sys/class/net/bond0/bonding/mode",
            shell=True,
            capture_output=True,
            text=True,
            check=True,
        ...
        tokens = mode.split()
        assert '4' in tokens or '802.3ad' in tokens, f'Unexpected bonding mode: {mode}'

        link_check = subprocess.run(
            f"sudo ip netns exec {ap['namespace']} ip -d link show bond0",
            shell=True,
            capture_output=True,
            text=True,
            check=True,
        ).stdout
        assert 'bond0' in link_check
        assert '802.3ad' in link_check or 'mode 4' in link_check
    finally:
        subprocess.run(
            f"sudo ip netns exec {ap['namespace']} ip link delete bond0 2>/dev/null",
            shell=True,
            check=False,
        )
