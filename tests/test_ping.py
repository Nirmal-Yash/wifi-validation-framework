...
def test_packet_loss_within_threshold(params):
    ...
    assert result["packet_loss_pct"] <= threshold,         f"Packet loss {result['packet_loss_pct']}% exceeds {threshold}%"

def test_latency_within_threshold(params):
    result = run_ping(ROUTER_IP)
    threshold = params["thresholds"]["max_latency_ms"]
    assert result["avg_rtt_ms"] <= threshold,         f"Latency {result['avg_rtt_ms']}ms exceeds {threshold}ms"
