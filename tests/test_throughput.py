...
def test_data_plane_throughput(system_config, test_params):
    ...
    if "inet 10.0.10." not in addr_out:
    ...
    server_cmd = f"sudo ip netns exec router_ns iperf3 -s -D"
    subprocess.run(server_cmd, shell=True, check=True)
    time.sleep(0.5)
    
    client_cmd = f"sudo ip netns exec {client['namespace']} iperf3 -c 10.0.10.1 -J"
