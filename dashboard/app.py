...
def fetch_live_gns3():
    ...
    payload=request.get_json(silent=True) or {}; server_url=str(payload.get("server_url") or "http://localhost:3080").strip().rstrip("/"); project_id=str(payload.get("project_id") or "").strip()
    if not project_id: return api_error("PROJECT_ID_REQUIRED","GNS3 Project UUID is required",422)
    try:
        project=Project(project_id=project_id,connector=Gns3Connector(url=server_url)); project.get(); nodes=[{"id":n.node_id,"label":n.name,"x":n.x or 0,"y":n.y or 0,"shape":"box","title":f"Device: {n.name}\\nType: {n.node_type}"} for n in project.nodes]; edges=[]
...
def save_topology():
    ...
    with connection() as conn:
        ...
        if existing: topology_id=existing["id"]; version=conn.execute("SELECT COALESCE(MAX(version),0)+1 v FROM topology_versions WHERE topology_id=?",(topology_id,)).fetchone()["v"]
        else: conn.execute("INSERT INTO topologies(name,source) VALUES(?,?)",(name,"local")); topology_id=conn.execute("SELECT last_insert_rowid()").fetchone()[0]; version=1
        ...
        for index,node in enumerate(nodes,1):
            label=str(node.get("label") or f"node-{index}").strip(); conn.execute("INSERT INTO topology_nodes(topology_version_id,node_key,name,node_type,x,y,metadata_json) VALUES(?,?,?,?,?,?,?)",(version_id,str(node.get("id",index)),label,infer_type(label),float(node.get("x",0)),float(node.get("y",0)),json.dumps(node))); id_map[node.get("id",index)]=conn.execute("SELECT last_insert_rowid()").fetchone()[0]
        for edge in payload.get("edges",[]):
            source,target=id_map.get(edge.get("from")),id_map.get(edge.get("to"))
            if source and target: conn.execute("INSERT INTO topology_links(topology_version_id,source_node_id,target_node_id,metadata_json) VALUES(?,?,?,?)",(version_id,source,target,json.dumps(edge)))
