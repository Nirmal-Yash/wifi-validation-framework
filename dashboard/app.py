...
def ensure_topology_seed():
    with connection() as conn:
        ...
        if not nodes: return
        conn.execute("INSERT INTO topologies(name,description,source) VALUES(?,?,?)",("default","Imported from config/devices.yaml","local")); topology_id=conn.execute("SELECT last_insert_rowid()").fetchone()[0]
        conn.execute("INSERT INTO topology_versions(topology_id,version,status) VALUES(?,?,?)",(topology_id,1,"ACTIVE")); version_id=conn.execute("SELECT last_insert_rowid()").fetchone()[0]
        for key,details in nodes.items():
            details=details or {}; conn.execute("INSERT INTO topology_nodes(topology_version_id,node_key,name,node_type,namespace,interface_name,config_path,x,y,metadata_json) VALUES(?,?,?,?,?,?,?,?,?,?)",(version_id,key,key,infer_type(key),details.get("namespace"),details.get("interface"),details.get("config_path"),details.get("x",0),details.get("y",0),json.dumps(details)))
        conn.commit()
def topology_payload(version_id=None):
    ensure_topology_seed()
    with connection() as conn:
        if version_id is None:
            version=conn.execute("SELECT tv.id,tv.topology_id,t.name,tv.version,tv.status,tv.created_at FROM topology_versions tv JOIN topologies t ON t.id=tv.topology_id WHERE tv.status='ACTIVE' ORDER BY tv.id DESC LIMIT 1").fetchone()
        else:
            version=conn.execute("SELECT tv.id,tv.topology_id,t.name,tv.version,tv.status,tv.created_at FROM topology_versions tv JOIN topologies t ON t.id=tv.topology_id WHERE tv.id=?",(version_id,)).fetchone()
...
def persist_topology(name,nodes,edges,source="local"):
    ...
    name=str(name or "default").strip()[:100] or "default"
    with connection() as conn:
        existing=conn.execute("SELECT id FROM topologies WHERE name=?",(name,)).fetchone()
        if existing:
            topology_id=existing["id"]; version=conn.execute("SELECT COALESCE(MAX(version),0)+1 v FROM topology_versions WHERE topology_id=?",(topology_id,)).fetchone()["v"]
        else:
            conn.execute("INSERT INTO topologies(name,source) VALUES(?,?)",(name,source)); topology_id=conn.execute("SELECT last_insert_rowid()").fetchone()[0]; version=1
        conn.execute("INSERT INTO topology_versions(topology_id,version,status) VALUES(?,?,?)",(topology_id,version,"ACTIVE")); version_id=conn.execute("SELECT last_insert_rowid()").fetchone()[0]; id_map={}
        for index,node in enumerate(nodes,1):
            key=str(node.get("node_key") or node.get("id",index)); label=str(node.get("label") or node.get("name") or key).strip(); conn.execute("INSERT INTO topology_nodes(topology_version_id,node_key,name,node_type,namespace,interface_name,config_path,x,y,metadata_json) VALUES(?,?,?,?,?,?,?,?,?,?)",(version_id,key,label,infer_type(label),node.get("namespace"),node.get("interface"),node.get("config_path"),float(node.get("x",0)),float(node.get("y",0)),json.dumps(node))); id_map[node.get("id",index)]=conn.execute("SELECT last_insert_rowid()").fetchone()[0]
        for edge in edges:
            source_id,target_id=id_map.get(edge.get("from")),id_map.get(edge.get("to"))
            if source_id and target_id: conn.execute("INSERT INTO topology_links(topology_version_id,source_node_id,target_node_id,metadata_json) VALUES(?,?,?,?)",(version_id,source_id,target_id,json.dumps(edge)))
        conn.execute("UPDATE topology_versions SET status='ARCHIVED' WHERE topology_id=? AND id<>? AND status='ACTIVE'",(topology_id,version_id)); conn.commit()
    return {"topology_id":topology_id,"version_id":version_id,"version":version,"name":name,"source":source}
@app.before_request
...
@app.route("/health")
def health():
    with connection() as conn: conn.execute("SELECT 1").fetchone()
...
def history():
    with connection() as conn:
        fw_list=conn.execute("SELECT firmware_version,MAX(timestamp) latest FROM test_logs GROUP BY firmware_version ORDER BY latest DESC").fetchall(); values=[r["firmware_version"] for r in fw_list]; fw_a=request.args.get("fw_a") or (values[-1] if values else ""); fw_b=request.args.get("fw_b") or (values[0] if values else fw_a)
        comparisons=[]
...
def list_topologies():
    with connection() as conn:
        rows=conn.execute("SELECT t.id,t.name,t.description,t.source,COUNT(tv.id) version_count,MAX(tv.version) latest_version FROM topologies t LEFT JOIN topology_versions tv ON tv.topology_id=t.id GROUP BY t.id ORDER BY t.id DESC").fetchall()
...
def get_topology_version(topology_id,version_id):
    with connection() as conn:
        row=conn.execute("SELECT id FROM topology_versions WHERE id=? AND topology_id=?",(version_id,topology_id)).fetchone()
        if not row: return api_error("TOPOLOGY_VERSION_NOT_FOUND","Topology version does not exist",404)
    return api_ok(topology_payload(version_id))
@app.route("/api/topology/validate",methods=["POST"])
def validate_topology():
    payload=request.get_json(silent=True) or {}; nodes,links=payload.get("nodes",[]),payload.get("edges",[])
    ...
    ids={n.get("id") for n in nodes}; invalid=[e for e in links if e.get("from") not in ids or e.get("to") not in ids]
    if invalid: return api_error("INVALID_LINK","One or more links reference missing nodes",422,{"count":len(invalid)})
...
def fetch_live_gns3():
    ...
    payload=request.get_json(silent=True) or {}; server_url=str(payload.get("server_url") or "http://localhost:3080").strip().rstrip("/"); project_id=str(payload.get("project_id") or "").strip(); persist=bool(payload.get("persist",True)); name=str(payload.get("name") or "GNS3 Import").strip()
    if not project_id: return api_error("PROJECT_ID_REQUIRED","GNS3 Project UUID is required",422)
    try:
        project=Project(project_id=project_id,connector=Gns3Connector(url=server_url)); project.get(); nodes=[{"id":n.node_id,"node_key":n.node_id,"label":n.name,"x":n.x or 0,"y":n.y or 0,"shape":"box","title":f"Device: {n.name}\\nType: {n.node_type}","metadata":{"gns3_node_id":n.node_id,"node_type":n.node_type}} for n in project.nodes]; edges=[]
        ...
        result={"nodes":nodes,"edges":edges,"source":"gns3","server_url":server_url,"project_id":project_id}
        if persist: result["commit"]=persist_topology(name,nodes,edges,"gns3")
        return api_ok(result)
    except Exception as exc: return api_error("GNS3_SYNC_FAILED",f"Failed to connect to GNS3: {exc}",502)
@app.route("/api/save_topology",methods=["POST"])
def save_topology():
    payload=request.get_json(silent=True) or {}; nodes=payload.get("nodes")
    if not isinstance(nodes,list): return api_error("INVALID_REQUEST","nodes array is required",422)
    try: result=persist_topology(payload.get("name") or "default",nodes,payload.get("edges",[]),"local")
    except (ValueError,TypeError) as exc: return api_error("INVALID_TOPOLOGY",str(exc),422)
    return api_ok({"message":"Topology committed","commit":result},201)
...
def manage_config(node_name):
    ...
    body=request.get_json(silent=True) or {}; content=body.get("content")
    if not isinstance(content,str): return api_error("INVALID_CONFIG","content must be a JSON string",422)
    try: parsed=json.loads(content)
    except json.JSONDecodeError as exc: return api_error("INVALID_JSON",str(exc),422)
...
def import_topology():
    ...
    try:
        importer=TopologyImporter(target_yaml=str(CONFIG_DIR/"devices.yaml")); importer.import_json(str(path)) if path.suffix.lower()==".json" else importer.import_gns3(str(path)); ensure_topology_seed(); return api_ok({"message":f"Successfully imported {filename}"})
