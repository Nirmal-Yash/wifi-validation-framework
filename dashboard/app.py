...
def ensure_topology_seed() -> None:
    """Create a DB topology from the legacy YAML only when the DB has none."""
    with connection() as conn:
        count = conn.execute("SELECT COUNT(*) AS c FROM topologies").fetchone()["c"]
        if count:
            return
        data = load_yaml_topology()
        ...
        for key, details in nodes.items():
            ...
            conn.execute(
                ...
                (topology_version_id,node_key,name,node_type,namespace,interface_name,config_path,x,y,metadata_json)
                VALUES(?,?,?,?,?,?,?,?,?,?)""",
                (version_id, key, key, infer_type(key), details.get("namespace"), details.get("interface"), details.get("config_path"), details.get("x", 0), details.get("y", 0), json.dumps(details)),
            )
        conn.commit()
...
def topology_payload() -> dict:
    ...
    with connection() as conn:
        ...
        for row in rows:
            ...
            nodes.append({"id": row["id"], "label": row["name"], "x": row["x"], "y": row["y"], "shape": "icon", "title": f"Device: {row['name']}\\nType: {kind}\\nNamespace: {row['namespace'] or 'N/A'}\\nInterface: {row['interface_name'] or 'N/A'}", "icon": {"face": '"Font Awesome 6 Free"', "code": icons.get(kind, "\\uf233"), "weight": 900, "size": 60}})
...
def save_topology():
    ...
    if not name:
        return api_error("INVALID_NAME", "Topology name cannot be empty", 422)
    with connection() as conn:
        existing = conn.execute("SELECT id FROM topologies WHERE name=?", (name,)).fetchone()
        if existing:
            topology_id = existing["id"]
            latest = conn.execute("SELECT COALESCE(MAX(version),0) v FROM topology_versions WHERE topology_id=?", (topology_id,)).fetchone()["v"]
            version = latest + 1
        else:
...
def manage_config(node_name: str):
    ...
    if request.method == "GET":
        ...
        default = {"hostname": node_name, "interfaces": [], "protocols": [], "status": "enabled"}
        return api_ok({"content": json.dumps(default, indent=2)})
