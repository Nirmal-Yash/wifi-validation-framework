# NetRegress — Architecture Decisions & Business Logic

> Canonical upstream design document for architectural decisions, product semantics, lifecycle meaning, evidence rules, release behavior, and business logic.
>
> Architecture decisions answer what the system must mean and why. Business Logic turns those decisions into explicit domain semantics. The implementation plan consumes this document; it does not redefine it.
...
## 2. Business logic and domain semantics
...
## 3. Decision-to-implementation relationship
...
Implementation
↓
Testing / Operations / REAL_LAB evidence

When documents disagree, architectural decisions govern future intent, business logic governs product semantics, technical architecture governs structure, data/API governs persisted/external contracts, and executable behavior remains authoritative until the corresponding migration phase is complete.
